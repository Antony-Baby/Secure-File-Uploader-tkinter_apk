...
class file_upload:
    def __init__(self) -> None:
        global frame_up
        frame_up= tk.Frame(root,bg="blue",width=900,height=600)
        ...
        self.label_path.place(x=450,y=150)
...
class download:
    def __init__(self):
        ...
        self.inp_filename.place(x=300,y=200)
    ...
    def verifyotp1(self):
        ...
        if e_otp == reference_file[self.file_name]["author"]:
            if not self.author:
                ...
                self.inp_otp2.place(x=300,y=450)
                self.btn_otp2 = tk.Button(frame_up,width=25,wraplength=200,padx=5,text="Verify",command=self.file_download)
                self.btn_otp2.place(x=250,y=500)
            else:
                self.file_download()
        else:
            print("\nInvalid Otp")
